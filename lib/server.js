...
async function refreshVitalsAsync() {
  ...
  try {
    ...
    const coresCmd = isLinux ? "nproc" : "/usr/sbin/sysctl -n hw.ncpu";
    ...
    if (isAppleSilicon) {
      ...
      // Intel Mac: try osx-cpu-temp (tiny SMC reader)
...
function getSystemVitalsOLD_DISABLED() {
  ...
  try {
    ...
    try {
      ...
      const pageSizeMatch = memInfoRaw.match(/page size of (\d+) bytes/); const pageSize = pageSizeMatch ? parseInt(pageSizeMatch[1], 10) : 4096; // Default macOS page size
    ...
    if (isAppleSilicon) {
      ...
      try {
        const home = require("os").homedir();
        const tempEnv = { ...process.env, PATH: `/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:${home}/bin:${process.env.PATH || ""}` };
        const temp = execSync(`osx-cpu-temp 2>/dev/null || ${home}/bin/osx-cpu-temp 2>/dev/null || echo ""`, { encoding: "utf8", env: tempEnv }).trim();
        if (temp && temp.includes("°")) {
          const tempMatch = temp.match(/([\d.]+)/);
          if (tempMatch && parseFloat(tempMatch[1]) > 0) {
            vitals.temperature = parseFloat(tempMatch[1]);
...
// === CUSTOM: Read resource-state.json for accurate hybrid-auth cost tracking ===
function getResourceState() {
  try {
    const rsPath = path.join(
      process.env.OPENCLAW_WORKSPACE || require("os").homedir() + "/.openclaw/workspace",
      "memory/resource-state.json"
    );
    return JSON.parse(fs.readFileSync(rsPath, "utf8"));
  } catch (e) {
    console.error("[ResourceState] Error:", e.message);
    return null;
  }
}
...
function getFullState() {
  ...
  // CUSTOM: Ensure hybrid auth overlay is applied even if tokenStats partially failed
  if (!tokenStats.hybridAuth) {
    const rs = getResourceState();
    if (rs && rs.apiSpend) {
      const avg = rs.apiSpend.averages || {};
      const planMoCost = 17;
      const subTotal = (rs.costBySessionType?.main_subscription || 0) + (rs.costBySessionType?.expired_subscription || 0);
      const daysUp = Math.max(1, Math.floor((Date.now() - new Date("2026-02-08").getTime()) / 86400000));
      const subSavings = Math.round(((subTotal / daysUp) * 30) - planMoCost);
      const savPct = subTotal > 0 ? Math.round((1 - planMoCost / ((subTotal / daysUp) * 30)) * 100) : 0;

      tokenStats.estCost = `$${(avg.daily_1d || 0).toFixed(2)}`;
      tokenStats.estCostFormatted = tokenStats.estCost;
      tokenStats.estMonthlyCost = `$${Math.round((avg.daily_1d || 0) * 30)}`;
      tokenStats.hybridAuth = true;
      tokenStats.realApiSpend = `$${(rs.apiSpend.total || 0).toFixed(2)}`;
      tokenStats.budgetUsed = `${rs.budget?.percentageSpent || 0}%`;
      if (tokenStats.savingsWindows) {
        const wins = { "24h": avg.daily_1d, "3dma": avg.daily_3d, "7dma": avg.daily_7d };
        for (const [key, daily] of Object.entries(wins)) {
          if (tokenStats.savingsWindows[key] && daily !== undefined) {
            tokenStats.savingsWindows[key].estCost = `$${(daily || 0).toFixed(2)}`;
            tokenStats.savingsWindows[key].estMonthlyCost = `$${Math.round((daily || 0) * 30)}`;
            tokenStats.savingsWindows[key].estSavings = `$${subSavings}/mo`;
            tokenStats.savingsWindows[key].savingsPercent = savPct;
          }
        }
      }
    }
...
function getCostBreakdown() {
  ...
  // CUSTOM: Overlay hybrid auth accurate cost data
  const rs = getResourceState();
  if (rs && rs.apiSpend) {
    const avg = rs.apiSpend.averages || {};
    const planMonthlyCost = 17; // $200/year ≈ $17/mo

    // Override the total cost with real API cost
    result.totalCost = avg.daily_1d || 0;

    // Add hybrid auth context to the modal
    result.hybridAuth = {
      enabled: true,
      realApiSpend: rs.apiSpend.total || 0,
      subscriptionEquivalent: rs.subscription?.equivalentApiCost || 0,
      budgetUsed: rs.budget?.percentageSpent || 0,
      byModel: rs.apiSpend.byModel || {},
      byType: rs.apiSpend.byType || {},
      note: "Token costs above reflect ALL sessions at Opus API rates. " +
            "Actual cost is much lower: main sessions are on Pro subscription (free). " +
            "Only crons/subagents use API keys.",
      warnings: rs.warnings || []
    };

    // Fix window projections
    // Each window shows: real API cost per day, projected monthly API spend
    // Savings = subscription value (what main sessions would cost at Opus API rates per month)
    // We use 7d average of total session cost as a stable "what Opus would cost" baseline
    const totalDailyEquiv = (rs.costBySessionType?.main_subscription || 0) + 
                            (rs.costBySessionType?.expired_subscription || 0);
    const daysActive = Math.max(1, Math.floor((Date.now() - new Date("2026-02-08").getTime()) / 86400000));
    const dailySubEquiv = totalDailyEquiv / daysActive;
    const monthlySubEquiv = dailySubEquiv * 30;

    const subSavings = Math.round(monthlySubEquiv - planMonthlyCost);
    const savPct = monthlySubEquiv > 0 ? Math.round(((monthlySubEquiv - planMonthlyCost) / monthlySubEquiv) * 100) : 0;
    const winMap = { "24h": avg.daily_1d, "3d": avg.daily_3d, "7d": avg.daily_7d };
    for (const [key, daily] of Object.entries(winMap)) {
      if (result.windows[key]) {
        result.windows[key].dailyAvg = daily || 0;
        result.windows[key].totalCost = (daily || 0) * result.windows[key].days;
        result.windows[key].monthlyProjected = (daily || 0) * 30;
        result.windows[key].monthlySavings = subSavings;
        result.windows[key].savingsPercent = savPct;
      }
    }
  }
...
function getTokenStats(sessions, capacity) {
  ...
  // CUSTOM: Override cost displays with accurate hybrid-auth data
  const rs = getResourceState();
  if (rs && rs.apiSpend) {
    const avg = rs.apiSpend.averages || {};
    const realDaily = avg.daily_1d || 0;
    const subEquiv = (rs.subscription?.equivalentApiCost || 0);

    // Override main cost display
    result.estCost = `$${formatNumber(realDaily)}`;
    result.estCostFormatted = `$${formatNumber(realDaily)}`;
    result.estMonthlyCost = `$${Math.round(realDaily * 30).toLocaleString()}`;

    // Savings = what subscription saves us (equivalent API cost - subscription price)
    const monthlySubCost = 17; // ~$200/year
    const monthlyEquiv = subEquiv / (new Date().getDate() / 30); // extrapolate to month
    const savings = monthlyEquiv - monthlySubCost;
    result.estSavings = savings > 0 ? `$${formatNumber(savings)}/mo` : null;
    result.savingsPercent = savings > 0 ? Math.round((savings / monthlyEquiv) * 100) : 0;

    // Override windows
    if (result.savingsWindows) {
      const wins = { "24h": avg.daily_1d, "3dma": avg.daily_3d, "7dma": avg.daily_7d };
      for (const [key, daily] of Object.entries(wins)) {
        if (result.savingsWindows[key] && daily !== undefined) {
          const mo = daily * 30;
          result.savingsWindows[key].estCost = `$${formatNumber(daily)}`;
          result.savingsWindows[key].estMonthlyCost = `$${Math.round(mo).toLocaleString()}`;
          // Savings in each window = equivalent subscription savings
          result.savingsWindows[key].estSavings = savings > 0 ? `$${formatNumber(savings)}/mo` : null;
          result.savingsWindows[key].savingsPercent = savings > 0 ? Math.round((savings / (monthlyEquiv || 1)) * 100) : 0;
        }
      }
    }

    // Add hybrid auth indicator
    result.hybridAuth = true;
    result.realApiSpend = `$${formatNumber(rs.apiSpend.total)}`;
    result.budgetUsed = `${rs.budget?.percentageSpent || 0}%`;
  }
...
function getLlmUsage() {
  ...
  // CUSTOM: Fall back to resource-state.json for budget-based usage display
  const rs = getResourceState();
  if (rs && rs.budget) {
    const b = rs.budget;
    const api = rs.apiSpend || {};
    const avg = api.averages || {};
    // Map our budget tracking to the panel's expected format
    // "Session Usage" → Daily budget (how much of $20/day used)
    // "Weekly Usage" → Weekly budget (how much of $100/week used)
    const dailySpent = avg.daily_1d || 0;
    const dailyPct = Math.min(100, Math.round((dailySpent / (b.daily || 20)) * 100));
    const weeklySpent = (avg.daily_7d || 0) * 7;
    const weeklyPct = Math.min(100, Math.round((weeklySpent / (b.weekly || 100)) * 100));
    const monthlyPct = Math.round(b.percentageSpent || 0);
    ...
    return {
      ...
      claude: {
        session: {
          usedPct: dailyPct,
          remainingPct: 100 - dailyPct,
          resetsIn: "midnight",
          label: "Daily Budget",
          detail: `$${dailySpent.toFixed(2)} of $${b.daily || 20}/day`,
        },
        weekly: {
          usedPct: weeklyPct,
          remainingPct: 100 - weeklyPct,
          resets: "Mon",
          label: "Weekly Budget",
          detail: `$${weeklySpent.toFixed(2)} of $${b.weekly || 100}/week`,
        },
        sonnet: {
          usedPct: monthlyPct,
          remainingPct: 100 - monthlyPct,
          resets: "1st",
          label: "Monthly Budget",
          detail: `$${(api.total || 0).toFixed(2)} of $${b.monthly || 300}/month`,
        },
        lastSynced: rs.timestamp || new Date().toISOString(),
      },
      codex: { sessionsToday: 0, tasksToday: 0, usage5hPct: 0, usageDayPct: 0 },
      routing: { total: 0, claudeTasks: 0, codexTasks: 0, claudePct: 0, codexPct: 0, codexFloor: 20 },
    };
  ...
  // No valid data - return auth error state
  return {
    timestamp: new Date().toISOString(),
    source: "error",
    ...
    errorType: "auth",
    claude: {
      session: { usedPct: null, remainingPct: null, resetsIn: null, error: "No data" },
      weekly: { usedPct: null, remainingPct: null, resets: null, error: "No data" },
      sonnet: { usedPct: null, remainingPct: null, resets: null, error: "No data" },
      lastSynced: null,
    },
    codex: { sessionsToday: 0, tasksToday: 0, usage5hPct: 0, usageDayPct: 0 },
    routing: { total: 0, claudeTasks: 0, codexTasks: 0, claudePct: 0, codexPct: 0, codexFloor: 20 },
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === "/api/status") {
    ...
    // getCostBreakdown() already applies hybrid auth overlay internally
    const data = getCostBreakdown();
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify(data, null, 2));
  } else if (pathname === "/api/resource-state") {
    // CUSTOM: Serve raw resource-state.json for custom widgets
    const rs = getResourceState();
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify(rs || { error: "Not found" }, null, 2));
